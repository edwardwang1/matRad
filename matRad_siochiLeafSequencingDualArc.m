...
for ii = 1:2
    for i = 1:numOfBeams/2
        new_index = (ii-1) * numOfBeams/2 + i;
        numOfRaysPerBeam = stf(new_index).numOfRays;
        
        if pln.propOpt.runVMAT
            
            if ~stf(new_index).propVMAT.FMOBeam
                ...
                numToKeep = stf(new_index).propVMAT.numOfBeamChildren;
            end
        else
        ...
        for j = 1:stf(new_index).numOfRays
            X(j) = stf(new_index).ray(j).rayPos_bev(:,1);
            Z(j) = stf(new_index).ray(j).rayPos_bev(:,3);
        end
        ...
        dimOfFluenceMxX = (maxX-minX)/stf(new_index).bixelWidth + 1;
        dimOfFluenceMxZ = (maxZ-minZ)/stf(new_index).bixelWidth + 1;
        ...
        xPos = (X-minX)/stf(new_index).bixelWidth+1;
        zPos = (Z-minZ)/stf(new_index).bixelWidth+1;
        ...
        while notFinished
            ...
            if visBool
                [shapes,shapesWeight,k,D_k]=matRad_siochiConvertToSegments(shapes,shapesWeight,k,tops,bases,visBool,new_index,D_k,numOfLevels,seqFig,seqSubPlots);
            else
