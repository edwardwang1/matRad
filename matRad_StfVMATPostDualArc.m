...
for i = 1:length(pln.propStf.gantryAngles)
    % Determine which FMO beam the current beam belongs to, considering the full cycle of gantry movement
    
    % Assuming the first half of pln.propStf.gantryAngles are for the first pass and the second half for the reverse pass
    midPointGantry = numel(pln.propStf.gantryAngles) / 2;
    midPointFMOGantry = numel(pln.propStf.FMOGantryAngles) / 2;
    if i <= midPointGantry
        % First pass
        [~,stf(i).propVMAT.beamParentFMOIndex] = min(abs(pln.propStf.FMOGantryAngles(1:midPointFMOGantry) - pln.propStf.gantryAngles(i)));
    else
        % Second pass - reverse direction
        [~,stf(i).propVMAT.beamParentFMOIndex] = min(abs(pln.propStf.FMOGantryAngles(midPointFMOGantry+1:end) - pln.propStf.gantryAngles(i)));
        stf(i).propVMAT.beamParentFMOIndex = stf(i).propVMAT.beamParentFMOIndex + midPointFMOGantry; % Adjust index for the second half
    end
    stf(i).propVMAT.beamParentGantryAngle = pln.propStf.FMOGantryAngles(stf(i).propVMAT.beamParentFMOIndex);
    % Find global index, considering all angles
    parentIndicies = find(abs(pln.propStf.gantryAngles - stf(i).propVMAT.beamParentGantryAngle) < 1e-6);
    if i <= midPointGantry
        stf(i).propVMAT.beamParentIndex = parentIndicies(1);
    else
        stf(i).propVMAT.beamParentIndex = parentIndicies(2);
    end

    %----------------Old code non dual arc
    ...
    elseif i == length(pln.propStf.gantryAngles)/2
    ...
    elseif i == length(pln.propStf.gantryAngles)/2 + 1
        stf(i).propVMAT.doseAngleBorders = [pln.propOpt.VMAToptions.finishingAngle (pln.propStf.gantryAngles(i+1)+pln.propStf.gantryAngles(i))/2];
    elseif i == length(pln.propStf.gantryAngles)
        stf(i).propVMAT.doseAngleBorders = [(pln.propStf.gantryAngles(i-1)+pln.propStf.gantryAngles(i))/2 pln.propOpt.VMAToptions.startingAngle];
    else        
    ...
    if stf(i).propVMAT.DAOBeam
        ...
        %has influence
        numDAOGantryAngles = numel(pln.propStf.DAOGantryAngles);
        midPointDAOGantryAngles = numDAOGantryAngles / 2;
        if i <= midPointGantry
            DAOIndex = find(abs(pln.propStf.DAOGantryAngles(1:midPointDAOGantryAngles) - pln.propStf.gantryAngles(i)) < 1e-8);
        else
            DAOIndex = find(abs(pln.propStf.DAOGantryAngles(midPointDAOGantryAngles+1:end) - pln.propStf.gantryAngles(i)) < 1e-8);
            DAOIndex = DAOIndex + midPointDAOGantryAngles;
        end
        ...
        if DAOIndex == 1
            ...
            nextDAOIndices = find(abs(pln.propStf.gantryAngles - pln.propStf.DAOGantryAngles(DAOIndex+1)) < 1e-8);
            if i <= midPointGantry
                nextDAOIndex = nextDAOIndices(1);
            else
                nextDAOIndex = nextDAOIndices(2);
            end
        ...
        elseif DAOIndex == length(pln.propStf.DAOGantryAngles)
            ...
            lastDAOIndices = find(abs(pln.propStf.gantryAngles - pln.propStf.DAOGantryAngles(DAOIndex-1)) < 1e-8);
            if i <= midPointGantry
                stf(i).propVMAT.lastDAOIndex = lastDAOIndices(1);
            else
                stf(i).propVMAT.lastDAOIndex = lastDAOIndices(2);
            end
    ...
    if stf(i).propVMAT.FMOBeam
        ...
        FMOIndicies = find(abs(pln.propStf.FMOGantryAngles - pln.propStf.gantryAngles(i)) < 1e-8);
        if i <= midPointGantry
            FMOIndex = FMOIndicies(1);
        else
            FMOIndex = FMOIndicies(2);
        end
